...
가역적인 체크포인트로 관리하는 로컬 우선(local-first) 플러그인입니다. 현재
저장소 파일과 테스트가 언제나 최종 진실이며, ContextGC의 Task Frame은 그
진실을 대신하는 데이터베이스가 아니라 작업을 이어 가기 위한 검증 가능한
인계 문서입니다.
...
- Git
- Node.js 22.13 이상
- 설치·발견 절차를 검증한 Codex CLI 0.144.5
...
- Codex CLI가 설치 검증 버전인 `0.144.5`이다. 다른 버전은
  [인터페이스 레퍼런스의 호환성 표](reference.md#compatibility-matrix)를
  확인하고 설치·MCP·skill·hook 발견 검사를 다시 수행한다.
...
hook의 contextgcStoreId와 MCP의 storeId만 비교하고 절대 경로는 요청하거나
알리거나 대화에 붙여 넣지 마.
...
redaction 상태와 누락되거나 오래된 정보를 보고해. 로컬 경로는 보고하지 말고
/compact를 실행하거나 credits를 추정하지 마.
```

기대 결과는 다음과 같습니다.

- hook과 MCP가 같은 16자리 16진수 opaque `storeId`를 보고하고 절대 store
  경로는 대화에 나타나지 않는다.
- 저장소에서 다시 확인한 사실만 Task Frame에 들어간다.
- 새 `checkpointId`가 반환된다.
- 상태 변경의 근거와 누락된 정보가 함께 보고된다.
- hook과 MCP가 동일한 opaque `storeId`를 사용한다.
...
try {
  $result = node $cli simulate --output $smokeRoot --compact | ConvertFrom-Json
...
체크인된 prebuilt 플러그인으로 업데이트하려면 설치된 캐시를 명확히 교체합니다.
...
다른 플러그인이 이 marketplace를 사용하지 않고 목록에서도 더 이상 필요하지
않다면 marketplace 등록도 제거할 수 있습니다.

```powershell
codex plugin marketplace remove context-gc-local --json
```

이 명령은 Git clone 자체를 삭제하는 명령이 아닙니다. clone을 보존하면 나중에
다시 설치하거나 기록을 검토할 수 있습니다.

### 10.2 체크포인트 데이터 보존 여부 결정

ContextGC 자체는 persisted object를 자동 삭제하지 않으며, 재귀 삭제 MCP
도구도 제공하지 않습니다. 그러나 Codex 플러그인 제거 과정이 특정 설치
환경의 data directory를 어떻게 취급하는지 추정해서는 안 됩니다. **제거
전에** 다음 정보를 기록하고 필요한 경우 백업하십시오.

1. `contextgc_status`가 반환한 opaque `storeId`
2. 최신 checkpoint ID
3. 필요한 `archive`, `checkpoints`, `events.jsonl`과 `latest.json`

삭제 또는 백업에 절대 경로가 필요하면 승인된 로컬 관리 절차에서만 확인합니다.
아래 CLI override는 고급 사용자용이며 출력 경로를 공유하지 마십시오.

```powershell
$dataDir = 'C:\absolute\locally-reviewed-private-store'
node scripts\contextgc.bundle.mjs status --data-dir $dataDir --compact
```

백업은 삭제보다 먼저 수행하십시오. 아래 대상은 조직이나 사용자가 승인했고
BitLocker, EFS 또는 동등한 저장장치 암호화가 실제로 켜진 **절대 경로**로
바꾸십시오. ContextGC는 대상 암호화를 확인하거나 백업을 암호화하지 않으며,
암호화된 대상이 아니면 복사본도 plaintext입니다.
...
$backupDir = Join-Path $approvedEncryptedBackupRoot `
  ('contextgc-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
$sourceManifest = @(Get-ChildItem -LiteralPath $sourceRoot -Recurse -File |
  ForEach-Object {
    [pscustomobject]@{
...
$backupManifest = @(Get-ChildItem -LiteralPath $copiedRoot -Recurse -File |
  ForEach-Object {
    [pscustomobject]@{
      Relative = $_.FullName.Substring($copiedRoot.Length + 1)
      Length   = $_.Length
      Hash     = (Get-FileHash -LiteralPath $_.FullName -Algorithm SHA256).Hash
    }
  } | Sort-Object Relative)
...
if (Compare-Object $sourceManifest $backupManifest -Property Relative, Length, Hash) {
  throw 'Backup verification failed; keep the original store unchanged.'
}
Write-Output "verifiedBackup=true files=$($sourceManifest.Count)"
```

데이터를 버리기로 결정했다면 플러그인을 먼저 제거하고, status에서 확인한
**정확한 dataDir 하나만** 파일 탐색기에서 검토한 뒤 삭제하십시오. 작업공간
루트, 사용자 홈, `~\.codex` 전체 또는 계산된 상위 경로를 재귀 삭제하지
마십시오. archive에서 탐지·치환된 secret bytes는 백업에도 존재하지 않으며
ContextGC로 복구할 수 없습니다.
...
`dataDir`를 생략하고 hook의 `contextgcStoreId`와 MCP의 `storeId`를 비교한 뒤
