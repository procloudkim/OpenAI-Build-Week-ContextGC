...
| DOC-005 | ContextGC `0.1.13` requires Node.js 22.13 or newer and exposes six MCP tools. | versioned | `package.json`, `.codex-plugin/plugin.json`, `src/mcp/server.ts` | 2026-07-23 | Source inspection, skill validator, MCP bundle smoke | High | Runtime floor or tool contract changes |
| DOC-006 | ContextGC does not initiate native compaction; its hook may permit or block a host-initiated boundary. It also provides no deterministic token-to-Codex-credit conversion. | current/interpretive | `src/mcp/server.ts`, `hooks/run-hook.mjs`, `README.md`, [OpenAI compaction guide](https://developers.openai.com/api/docs/guides/compaction) | 2026-07-23 | Source inspection, hook tests, and official documentation review | High | Compaction actuation, hook control, or credit API changes |
...
| DOC-010 | Normal installed-plugin MCP calls may omit `dataDir`; MCP results use `storeId`, injected Task Frames use `contextgcStoreId`, and neither exposes the absolute root. | versioned/security | `src/mcp/server.ts`, `src/runtime/paths.ts`, MCP and hook tests | 2026-07-23 | Source inspection and focused tests | High | Store resolution or result schema changes |
...
| DOC-017 | In ContextGC `0.1.13`, healthy prompt, tool, and Stop hooks are user-silent; Stop never creates a model continuation, freshness alone cannot block automatic compaction, and every user-visible hook notice is limited to three lines and 240 characters. | versioned/UX | `hooks/run-hook.mjs`, `hooks/hooks.json`, `tests/hooks.test.ts` | 2026-07-23 | Focused lifecycle regression tests plus installed-cache parity | High | Hook output contract or CLI rendering changes |
| DOC-018 | Locally observed Codex CLI `0.145.0` stable transcript envelopes and token categories match the guarded ContextGC adapter; later `0.145.x` patches remain unverified. | versioned | `src/runtime/telemetry.ts`, `tests/fixtures/codex-0.145.0-transcript.jsonl`, `tests/runtime.test.ts` | 2026-07-23 | Local shape-only transcript inspection plus sanitized fixture regression | High for the observed `0.145.0` shape only | Codex CLI or transcript schema changes |
| DOC-019 | Host-managed Codex memories are advisory recall and are not read, modified, deduplicated, or accepted as checkpoint evidence by ContextGC. | current/interpretive | `README.md`, [Memories](https://learn.chatgpt.com/docs/customization/memories), ContextGC source inspection | 2026-07-23 | Official documentation and source boundary review | High | Native memory or ContextGC integration changes |
| DOC-020 | The hosted evidence explorer returned HTTP `200` without authentication on 2026-07-23; this is a dated availability observation, not a future guarantee. | volatile | `submission/judge-guide.md`, hosted demo URL | 2026-07-23 | Unauthenticated HTTP GET and content marker check | Medium | Hosting, access policy, or DNS changes |
| DOC-021 | The `v0.1.13` release records the reviewed two bundles, hook runner, hook manifest, and plugin manifest in `release/v0.1.13.sha256`; public release identity is established only when the public tag resolves to this reviewed tree. | versioned/security | `release/v0.1.13.sha256`, release tag | 2026-07-23 | Local SHA-256 calculation plus public tag and raw-manifest verification | High | Any recorded artifact or tag changes |
| DOC-022 | Root and site audits report zero known vulnerabilities with `fast-uri` 3.1.4, `sharp` 0.35.3, `@hono/node-server` 2.0.11, and Next.js 16.2.11; CI blocks moderate-or-higher findings. | volatile/security | `package.json`, `site/package.json`, both lockfiles, `.github/workflows/ci.yml`, GitHub Actions | 2026-07-23 | Both `npm audit --audit-level=moderate` commands plus full verify | High for the locked trees; volatile over time | Dependency or advisory changes |
| DOC-023 | The exact Hono override is outside the MCP SDK 1.29.0 declared `^1.19.9` range. Hono's v2 release retains the public `getRequestListener` API and requires Node 20+, while ContextGC requires Node 22.13+; a loopback compatibility test exercises that API. | versioned/security | [Hono v2 release](https://github.com/honojs/node-server/releases/tag/v2.0.0), [GHSA-frvp-7c67-39w9](https://github.com/advisories/GHSA-frvp-7c67-39w9), `package.json`, `tests/mcp.test.ts` | 2026-07-23 | Primary-source review, installed-version inspection, compatibility test, full verify | Medium until the MCP SDK natively adopts Hono 2.x | MCP SDK dependency range or Hono public API changes |

## Runnable example receipts

| Artifact | Purpose | Expected observable | Verification path |
| --- | --- | --- | --- |
| `node scripts/contextgc.bundle.mjs --version` | Confirm bundled CLI version | `0.1.13` | CLI test and bundle smoke |
| `node scripts/contextgc.bundle.mjs simulate` | Replay frozen fixtures without installing the plugin | Receipt hash `f7699823546f79657aea0faa290c0c648b8876236456f7a8ff02003875147ddd` | `output/benchmark/benchmark-report.json` |
