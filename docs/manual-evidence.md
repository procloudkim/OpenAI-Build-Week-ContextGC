...
| DOC-005 | ContextGC `0.1.11` requires Node.js 22.13 or newer and exposes six MCP tools. | versioned | `package.json`, `.codex-plugin/plugin.json`, `src/mcp/server.ts` | 2026-07-23 | Source inspection, skill validator, MCP bundle smoke | High | Runtime floor or tool contract changes |
...
| DOC-021 | The `v0.1.11` release records the reviewed two bundles, hook runner, hook manifest, and plugin manifest in `release/v0.1.11.sha256`; public release identity is established only when the public tag resolves to this reviewed tree. | versioned/security | `release/v0.1.11.sha256`, release tag | 2026-07-23 | Local SHA-256 calculation plus public tag and raw-manifest verification | High | Any recorded artifact or tag changes |
...
| --- | --- | --- | --- |
| `node scripts/contextgc.bundle.mjs --version` | Confirm bundled CLI version | `0.1.11` | CLI test and bundle smoke |
