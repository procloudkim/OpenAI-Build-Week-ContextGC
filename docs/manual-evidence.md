...
| DOC-005 | ContextGC `0.1.9` requires Node.js 22.13 or newer and exposes six MCP tools. | versioned | `package.json`, `.codex-plugin/plugin.json`, `src/mcp/server.ts` | 2026-07-23 | Source inspection, skill validator, MCP bundle smoke | High | Runtime floor or tool contract changes |
...
| DOC-021 | The `v0.1.9` release candidate records the reviewed two bundles, hook runner, hook manifest, and plugin manifest in `release/v0.1.9.sha256`; public release identity is established only when the public tag resolves to this reviewed tree. | versioned/security | `release/v0.1.9.sha256`, release tag | 2026-07-23 | Local SHA-256 calculation completed; public tag verification pending at document freeze | High for manifest consistency; pending for public tag identity | Any recorded artifact or tag changes |

## Runnable example receipts

| Artifact | Purpose | Expected observable | Verification path |
| --- | --- | --- | --- |
| `node scripts/contextgc.bundle.mjs --version` | Confirm bundled CLI version | `0.1.9` | CLI test and bundle smoke |
