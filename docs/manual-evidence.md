...
| DOC-005 | ContextGC `0.1.6` requires Node.js 22.13 or newer and exposes six MCP tools. | versioned | `package.json`, `.codex-plugin/plugin.json`, `src/mcp/server.ts` | 2026-07-21 | Source inspection, skill validator, MCP bundle smoke | High | Runtime floor or tool contract changes |
...
| DOC-017 | In ContextGC `0.1.6`, healthy prompt, tool, and Stop hooks are user-silent; Stop never creates a model continuation, and every user-visible hook notice is limited to three lines and 240 characters. | versioned/UX | `hooks/run-hook.mjs`, `hooks/hooks.json`, `tests/hooks.test.ts` | 2026-07-21 | Focused lifecycle regression tests plus installed-cache parity | High | Hook output contract or CLI rendering changes |

## Runnable example receipts

| Artifact | Purpose | Expected observable | Verification path |
| --- | --- | --- | --- |
| `node scripts/contextgc.bundle.mjs --version` | Confirm bundled CLI version | `0.1.6` | CLI test and bundle smoke |
